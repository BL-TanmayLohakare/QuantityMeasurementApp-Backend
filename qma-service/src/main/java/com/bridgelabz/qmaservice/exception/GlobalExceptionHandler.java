package com.bridgelabz.qmaservice.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseDTO> handleDatabaseException(DatabaseException ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ResponseDTO("Database Error: " + ex.getMessage(), null));
    }

    // Handle illegal arguments (like unknown category or unit)
    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ResponseDTO> handleIllegalArgumentException(IllegalArgumentException ex) {
        logger.warn("Invalid argument provided: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ResponseDTO("Invalid Argument: " + ex.getMessage(), null));
    }
